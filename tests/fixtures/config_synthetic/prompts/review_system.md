...
1. **Missing or superfluous relationships.** Identifying every statistical relationship a study reports, and deciding which of them meet the criteria for extraction, is genuinely difficult. Where there are errors, call `add_record` or `remove_record`.
