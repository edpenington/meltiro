...
The reviewer is given the paper's text, the cropped tables and figures as attached images, each labelled in that message, and the assembled extraction output. The output was produced by an extractor working from the same material, against a schema the reviewer also has access to. For each field the extractor provided a proposed value and evidence from the source text.
...
{include_if:checker:meltiro:reviewer_checker_feedback}
...
`mark_complete` is the normal ending, called once the output is correct and the reviewer is satisfied, and it may be called in the same turn as the final revisions. Its required `quality_check` argument carries the reviewer's own account of how the review went, in the same bare-value shape as the output's other check fields: the issues that arose during it, the number of records the output ends with and how that compares to what the reviewer would have expected from this paper, and any general observation about the paper or the extraction. It is recorded under the reviewer's name, beside the extractor's and never over it, and the extractor's own answers are not shown. `mark_complete` also takes an optional `summary` for the session audit log, and it always ends the review: it is the only exit from this loop, so it is never refused on the content of the quality check.
