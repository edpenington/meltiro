...
each image under its label, the paper's caption for it and any footnote the
exhibit prints, which is where a role learns what an `<img>` citation may
name — and, where the role is sent
...
def image_label_text(label, captions=None, notes=None):
    ...
    without guessing; then, on the line below, the exhibit's printed footnote
    where the manifest declared one, so the definitions and units a table
    states under itself are readable as text and not only as pixels. The crop
    shows those lines too — this is the same words in a form the model does
    not have to read off an image.
...
def render_user_prompt_text(study_id, paper_text, image_labels,
    ...
    exhibit notes and the no-exhibits notice included.
...
def build_initial_user_blocks(study_id, paper_text, figures,
    ...
    Args:
        ...
        image_notes: label -> footnote map (`PaperBundle.exhibit_notes`), so
            an exhibit that prints a footnote arrives with its text beside
            the caption.
...
def build_review_user_blocks(study_id, paper_text, figures,
    ...
    label, the paper's caption for it and any footnote it prints, + the
    assembled extraction output as a JSON block, framed as "review and confirm
    or revise". With no images to attach it reads `NO_EXHIBITS_NOTICE` where
    they would have been, on the same terms as the extractor. Last block
