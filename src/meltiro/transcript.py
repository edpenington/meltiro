...
class _Renderer:
    ...
    def _render_instrument(self):
        ...
        self._render_prompt(
            ...
            "and each attached exhibit's label, caption and printed notes. "
            "The figures themselves went alongside it as image parts, and "
            "are listed in 2.6.",
        )
        self._render_prompt(
    ...
    def _render_figures(self):
        ...
        # footnote printed under it, which are the halves a label cannot carry
        # and the bundle directory need no longer hold. A bare string is a
        # label on its own, from a session whose record keeps no captions; it
        # is read as an exhibit whose caption is unknown, and the cell says so
        # rather than the section failing to render. An entry from before the
        # notes key existed reads the same way in its own cell.
        ...
        if not exhibits:
            ...
            rows = [[f"`{e['label']}`",
                    ...
                    for e in exhibits]
            self._block(_table(
                ["Label", "Caption", "Notes", "SHA-256 of the cropped PNG",
                 "Bytes"],
                rows))
